...
import { createElement, editElement, setCurrentTab } from '@redux/slices/users/forms-slice.js'
...
import style from '@assets/components/orders/table.css?inline'
...
class Table extends HTMLElement {
  ...
  createOrderRow () {
    ...
    store.getState().users.users.forEach(order => {
      ...
      editButton.classList.add('edit-button', 'transition-colors', `edit-${order.id}`)
      editButton.innerHTML = `${orderEditSvg}`
      div.appendChild(editButton)
  
      const deleteButton = document.createElement('a')
      deleteButton.classList.add('delete-button', 'transition-colors', `delete-${order.id}`)
      deleteButton.innerHTML = `${orderDeleteSvg}`
      div.appendChild(deleteButton)
      
      imageCell.src = 'https://i.ibb.co/WDjgdXb/user-icon.png'
      imageCell.alt = 'user-icon'
  ...
  setupEventListeners () {
    ...
    this.shadow.querySelector('#create-button').addEventListener('click', () => {
      const inputs = store.getState().users_forms.inputs
  ...
  handleDelete (order, orderCard) {
    ...
    document.querySelector('#popup-component').shadowRoot
      ...
      .addEventListener('click', async () => {
        try {
          const response = await fetch(`http://localhost:8080/api/admin/users/${order.id}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
          })
          if (response.ok) {
            store.dispatch(removeUser(order.id))
...
customElements.define('table-users-component', Table)
