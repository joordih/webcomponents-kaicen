...
import { addElement, setCurrentTab } from '@redux/slices/orders/forms-slice.js'
import { setQueuedUpdate, incrementCount } from '@redux/slices/orders/orders-slice'
...
class Forms extends HTMLElement {
  ...
  setupStateSubscription () {
    this.unsubscribe = store.subscribe(() => {
      ...
      this.updateInputValues(state.orders_forms.inputs)
      this.updateTabVisibility(state.orders_forms.currentTab)
    })
  }
  ...
  async handleActionClick (event) {
    event.preventDefault()
    const currentTab = store.getState().orders_forms.currentTab
  ...
  async createOrder () {
    ...
    if (response.ok) {
      console.log(store.getState().orders_forms.currentTab)
...
customElements.define('forms-orders-component', Forms)
